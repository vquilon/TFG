...
exports.fesmo2 = function (req, res, next){
  //FECHA Y HORA
  ...
  mongo.connect(urlMongdb, function(err, db){
    if(err) throw err;
    db.collection('fesmos').findAndModify(
      {name:"DevicesDeployments"},
      [['_id','asc']],
      {$set: {"info":info, "data":data}},
      //Crea uno nuevo si no encuentra ningun fesmo(si esta vacio)
  ...
  /*fesmo.update({name:"DevicesDeployments"}, {info:info, data:data}, function(err){
    if(err){
...
exports.readMongo = function (req, res, next){
 //ANTIGUO CON LA BD ABIERTA EN APP
 /*fesmo.find(function(err, fesmo){
    console.log("Tipo: "+ typeof(fesmo));
    var f = JSON.stringify(fesmo);
    console.log("Tipo2: "+typeof(f));
    //console.log(JSON.parse(fesmo));
    if (err){
  ...
  mongo.connect(urlMongdb, function(err, db){
    if(err) throw err;
    var cursor = db.collection('fesmos').find();
...
/*mongoClient.connect(urlMongdb, function(err, db) {
      if (err) {
          ...
          var collection = db.collection('fesmo');
...
/*
  ...
  -exposedBy(determina un service(iot-lite))
    ...
    Puede ser util, se puede agregar un endpoint al fesmo y que este la opción también
  ...
  |Device:type especific| El sustema es un Device y tiene caracteristicas de system y device
